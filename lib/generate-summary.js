...
async function generateSummary(message, apiToken, opts) {
    ...
    const content = {
        ...
        attachments: []
            ...
            .map(attachment =>
                ({ filename: attachment.filename, contentType: attachment.contentType }.filter(attachment => attachment.filename || attachment.contentTyp))
            )
    };
    ...
    try {
        ...
        let walkAndRemoveNull = branch => {
            ...
            for (let key of Object.keys(branch)) {
                ...
                } else if (!['boolean', 'string', 'number'].includes(typeof subBranch)) {
                    delete branch[key];
                }
            }
        };
