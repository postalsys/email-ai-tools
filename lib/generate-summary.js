...
- In the event object, include "description" property that describes the event
- In the event object, include "location" property that defines the expected location of the event
- In the event object, include "startTime" property that includes a timestamp without a timezone for the start of the event
- In the event object, include "endTime" property that includes a timestamp without timezone for the expected end of the event if there is high confidence for the value
...
- For timestamps without timezone, use the "YYYY-MM-DD hh:mm:ss" format
...
${JSON.stringify(content)}`;
    ...
    try {
        ...
        if (typeof values.shouldReply !== 'undefined' && typeof values.shouldReply !== 'boolean') {
            delete values.shouldReply;
        }
        if (values && values.events) {
            values.events = []
                .concat(values.events || [])
