...
const USER_PROMPT = `
Instructions:
- You are an executive assistant scanning incoming emails to inform your superiors about what is important and what is not, and also to inform about obvious fraud attempts.
...
- The email includes a "subject" property for the email subject
...
${JSON.stringify(content)}`;
    ...
    try {
                    ...
                    for (let key of Object.keys(event)) {
                        switch (key) {
                            ...
                            case 'endTime':
                                {
                                    if (event[key]) {
                                        let parsedDate = new Date(event[key]);
        ...
        if (values && values.actions) {
            values.actions = [].concat(values.actions || []).map(action => {
                let result = {};
                for (let key of Object.keys(action)) {
                    switch (key) {
                        case 'dueDate':
                            {
                                if (action[key]) {
                                    let parsedDate = new Date(action[key]);
                                    if (parsedDate.toString() === 'Invalid Date') {
                                        action[key] = '';
                                    }
                                }
